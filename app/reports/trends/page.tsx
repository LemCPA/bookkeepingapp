...
export default function TrendsReport() {
  ...
  const fetchTrend = async () => {
    ...
    const res = await fetch(`/api/reports/trends?clientId=${selectedClient}&startMonth=${startMonth}&endMonth=${endMonth}&type=trend`)
    if (res.ok) {
  ...
  const fetchComparison = async () => {
    ...
    const res = await fetch(`/api/reports/trends?clientId=${selectedClient}&period1=${period1}&period2=${period2}&type=period`)
    if (res.ok) {
      const data = await res.json()
  ...
  const fetchYoY = async () => {
    ...
    const res = await fetch(`/api/reports/trends?clientId=${selectedClient}&year=${yoyYear}&type=yoy`)
    if (res.ok) {
      const data = await res.json()
